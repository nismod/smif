...
def down_migrations(working_path, database_name):
	...
	if isfile(join(working_path, file)):
		...
		subprocess.run(['psql', '-U', 'vagrant', '-d', '%s' %database_name, '-q', '-f', join(working_path, file)])
	else:
		return False
...
def up_migrations(working_path, database_name):
	...
	# run key file to add foreign keys between relations
	# run sql file silently
	subprocess.run(['psql', '-U', 'vagrant', '-d', '%s' %database_name, '-q', '-f', join(working_path, fky_file)])
...
def main():
	...
	cmd = """psql -U vagrant -tc "SELECT 1 FROM pg_database WHERE datname = '%s';" | grep -q 1 || psql -U vagrant -c "CREATE DATABASE %s;" """ %(db_name, db_name)
