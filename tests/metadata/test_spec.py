...
class TestSpec:
    ...
    def test_dim_elements(self, spec):
        ...
        assert spec.dim_elements("countries") == [
            {"name": "England"},
            {"name": "Wales"},
        ]
        ...
        spec = Spec.from_dict(
            {
                ...
                "description": "Population by age class",
                "dims": ["countries", "age"],
                "coords": {"age": [">30", "<30"], "countries": ["England", "Wales"]},
                "dtype": "int",
                "abs_range": (0, float("inf")),
                "exp_range": (10e6, 10e9),
                "unit": "people",
    ...
    def test_empty_dtype_error(self):
        ...
        with raises(ValueError) as ex:
            Spec(name="test", coords=[Coordinates("countries", ["England", "Wales"])])
    ...
    def test_coords_from_dict_error(self):
        ...
        with raises(ValueError) as ex:
            Spec(name="test", dtype="int", coords={"countries": ["England", "Wales"]})
        ...
        with raises(ValueError) as ex:
            Spec(
                ...
                dtype="int",
                dims=["countries"],
                coords={"countries": ["England", "Wales"], "age": [">30", "<30"]},
            )
        assert "dims must match the keys in coords" in str(ex.value)
