...
def config_data():
    ...
    sos_model.before_model_run = Mock(return_value=nx.DiGraph({
        'before_model_run_model_a': {},
        'before_model_run_model_b': {}
    }))
    sos_model.simulate = Mock(return_value=nx.DiGraph({
        'simulate_0_2010_model_a': {},
        'simulate_0_2010_model_b': {}
    }))
...
class TestModelRunnerJobGraphs():
    """Check if the ModelRunner glues jobgraphs generated by the SosModel 
    correctly together
    """
    def test_jobgraph_linear(self, mock_store, mock_model_run):
        """
        0   (before_model_run_model_a)
        |
        0   (simulate_model_b)
        """
        mock_model_run.sos_model.before_model_run = Mock(return_value=nx.DiGraph({
            'before_model_run_model_a': {}
        }))
        mock_model_run.sos_model.simulate = Mock(return_value=nx.DiGraph({
            'simulate_0_2010_model_a': {}
        }))
        ...
        assert list(job_graph.predecessors('before_model_run_model_a')) == []
        assert list(job_graph.successors('before_model_run_model_a')) == ['simulate_0_2010_model_a']

        assert list(job_graph.predecessors('simulate_0_2010_model_a')) == ['before_model_run_model_a']
        assert list(job_graph.successors('simulate_0_2010_model_a')) == []

    def test_jobgraph_convergent(self, mock_store, mock_model_run):
        """
        0   0  (before_model_run_model_a, before_model_run_model_b)
         \ /
          0    (simulate_model_a)
          |
          0    (simulate_model_b)
        ...
        assert list(job_graph.predecessors('before_model_run_model_b')) == []
        assert list(job_graph.successors('before_model_run_model_b')) == ['simulate_0_2010_model_a']

        assert sorted(list(job_graph.predecessors('simulate_0_2010_model_a'))) == sorted(['before_model_run_model_a', 'before_model_run_model_b'])
        assert list(job_graph.successors('simulate_0_2010_model_a')) == ['simulate_0_2010_model_b']

        assert list(job_graph.predecessors('simulate_0_2010_model_b')) == ['simulate_0_2010_model_a']
        assert list(job_graph.successors('simulate_0_2010_model_b')) == []

    def test_jobgraph_cross(self, mock_store, mock_model_run):
        """
        0   0  (before_model_run_model_a, before_model_run_model_b)
        | X |
        0   0  (simulate_model_a, simulate_model_b)
        """
        mock_model_run.sos_model.before_model_run = Mock(return_value=nx.DiGraph({
            'before_model_run_model_a': {},
            'before_model_run_model_b': {}
        ...
        assert list(job_graph.predecessors('before_model_run_model_a')) == []
        assert sorted(list(job_graph.successors('before_model_run_model_a'))) == sorted(['simulate_0_2010_model_a', 'simulate_0_2010_model_b'])

        assert list(job_graph.predecessors('before_model_run_model_b')) == []
        assert sorted(list(job_graph.successors('before_model_run_model_b'))) == sorted(['simulate_0_2010_model_a', 'simulate_0_2010_model_b'])

        assert sorted(list(job_graph.predecessors('simulate_0_2010_model_a'))) == sorted(['before_model_run_model_a', 'before_model_run_model_b'])
        assert list(job_graph.successors('simulate_0_2010_model_a')) == []

        assert sorted(list(job_graph.predecessors('simulate_0_2010_model_b'))) == sorted(['before_model_run_model_a', 'before_model_run_model_b'])
        assert list(job_graph.successors('simulate_0_2010_model_b')) == []