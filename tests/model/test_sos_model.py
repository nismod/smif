...
class TestSosModel:
    ...
    def test_construct(
        self, sos_model_dict, scenario_model, sector_model, economic_model
    ):
        ...
        assert sos_model.sector_models == [
            sector_model,
            economic_model,
        ] or sos_model.sector_models == [economic_model, sector_model]
    ...
    def test_add_dependency(self, empty_sector_model):
        ...
        sos_model.add_dependency(
            source_model, "hourly_value", sink_model, "hourly_value"
        )
...
class TestSosModelDependencies(object):
    ...
    def test_dependency_timestep(self, sos_model, scenario_model, sector_model):
        ...
        sos_model.add_dependency(
            scenario_model, "reservoir_level", sector_model, "reservoir_level"
        )

    def test_dependency_duplicate(self, sos_model, scenario_model, sector_model):
        with raises(SmifValidationError) as ex:
            sos_model.add_dependency(
                scenario_model, "precipitation", sector_model, "precipitation"
            )
        assert (
            "Could not add dependency: input 'precipitation' already provided"
            in str(ex.value)
        )
    ...
    def test_undefined_unit_conversion(
        self, sos_model_dict, sector_model, scenario_model, economic_model
    ):
        ...
        with raises(ValueError) as ex:
            SosModel.from_dict(
                sos_model_dict, [sector_model, scenario_model, economic_model]
            )
    ...
    def test_scenario_variant_not_present(
        self, sos_model_dict, scenario_model, economic_model
    ):
        ...
        assert (
            "SectorModel or ScenarioModel sink `water_supply` required "
            + "by dependency `economic_model (gva) - water_supply (rGVA)` "
            + "was not provided by the builder"
            in str(ex.value)
        )

    def test_scenario_model_not_provided(
        self, sos_model_dict, sector_model, economic_model
    ):
        """Error on scenario not provided"""
        with raises(SmifDataMismatchError) as ex:
            SosModel.from_dict(sos_model_dict, [sector_model, economic_model])
        assert (
            "SectorModel or ScenarioModel source `climate` required by "
...
class TestNarratives:
    ...
    def test_add_narrative_raises_for_incorrect_parameter(self, sos_model, narrative):
        ...
        assert "Parameter 'no_such_parameter' does not exist in 'water_supply'" in str(
            ex.value
        )
