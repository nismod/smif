...
class TestStoreConfig:
    ...
    def test_models_skip_coords(
        self, store, get_sector_model, get_sector_model_no_coords
    ):
        # write
        store.write_model(get_sector_model)
        # read all
        ...
        # read one
        assert (
            store.read_model(get_sector_model["name"], skip_coords=True)
    ...
    def test_scenarios_skip_coords(self, store, scenario, scenario_no_coords):
        ...
        # read one
        assert (
            store.read_scenario(scenario["name"], skip_coords=True)
            == scenario_no_coords
        )
        # update
        store.update_scenario(scenario["name"], scenario)
    ...
    def test_scenario_variants(self, store, scenario):
        ...
        new_variant = {
            ...
            "data": {"mortality": "mortality_high.csv"},
        }
        ...
        # read one
        assert (
            store.read_scenario_variant(scenario_name, old_variant["name"])
            == old_variant
        )
        # update
    ...
    def test_prepare_model_runs(
        self, store, model_run, sample_scenarios, sample_dimensions
    ):
        for dim in sample_dimensions:
            store.write_dimension(dim)
        ...
        assert (
            list_of_mr[1]["name"]
            == model_run["name"] + "_" + scenario["variants"][0]["name"]
        )
        assert (
            list_of_mr[2]["name"]
            == model_run["name"] + "_" + scenario["variants"][1]["name"]
        )
...
class TestStoreData:
    ...
    def setup(self, store, sample_dimensions, scenario, sample_scenario_data):
        for dim in sample_dimensions:
            store.write_dimension(dim)
        store.write_scenario(scenario)
    ...
    def test_scenario_variant_data(self, store, setup):
        ...
        assert store.read_scenario_variant_data(
            scenario_name, variant_name, variable, 2015, assert_exists=True
        )
        # Read 2015
    ...
    def test_convert_scenario_data(
        self,
        empty_store,
        store,
        sample_dimensions,
    ...
    def test_narrative_variant_data(
        self,
        store,
        sample_dimensions,
        get_sos_model,
    ...
    def test_convert_narrative_data(
        self,
        empty_store,
        store,
        sample_dimensions,
    ...
    def test_model_parameter_default(
        self, store, get_multidimensional_param, get_sector_model, sample_dimensions
    ):
        param_data = get_multidimensional_param
        for dim in sample_dimensions:
            store.write_dimension(dim)
        for dim in param_data.dims:
            store.write_dimension(
                {"name": dim, "elements": param_data.dim_elements(dim)}
            )
        ...
        assert store.read_model_parameter_default(
            get_sector_model["name"], param_data.name, assert_exists=True
        )
        # read
        actual = store.read_model_parameter_default(
            get_sector_model["name"], param_data.name
        )
        assert actual == param_data

    def test_convert_model_parameter_default_data(
        self,
        empty_store,
        store,
        ...
        src_store.convert_model_parameter_default_data(
            get_sector_model["name"], tgt_store
        )
    ...
    def test_interventions(
        self, store, sample_dimensions, get_sector_model, interventions
    ):
        # setup
        for dim in sample_dimensions:
            store.write_dimension(dim)
    ...
    def test_convert_interventions_data(
        self, empty_store, store, sample_dimensions, get_sector_model, interventions
    ):
        src_store = store
        tgt_store = empty_store
    ...
    def test_read_write_interventions_file(
        self, store, sample_dimensions, get_sector_model, interventions
    ):
        # setup
        for dim in sample_dimensions:
    ...
    def test_initial_conditions(
        ...
        # read
        assert (
            store.read_initial_conditions(get_sector_model["name"])
            == initial_conditions
        )
    ...
    def test_convert_initial_conditions_data(
        self,
        empty_store,
        store,
        sample_dimensions,
    ...
    def test_read_write_initial_conditions_file(
        self, store, sample_dimensions, get_sector_model, initial_conditions
    ):
        # setup
        for dim in sample_dimensions:
            store.write_dimension(dim)
        ...
        store.write_model(get_sector_model)
        # write
        store.write_initial_conditions_file(
            get_sector_model["name"], "path", initial_conditions
        )
        assert store.read_initial_conditions_file(
            get_sector_model["name"], "path", assert_exists=True
        )
    ...
    def test_conversion_coefficients(self, store, conversion_coefficients):
        # write
        store.write_coefficients("source_dim", "sink_dim", conversion_coefficients)
        # read
        numpy.testing.assert_equal(
            store.read_coefficients("source_dim", "sink_dim"), conversion_coefficients
        )
    ...
    def test_results(self, store, sample_results):
        ...
        assert (
            store.read_results("model_run_name", "model_name", spec, 0)
            == sample_results
        )
        # check
    ...
    def test_filter_complete_available_results(self, store):
        ...
        expected = [(2020, 0, "test_model")]
        actual = store.filter_complete_available_results(
