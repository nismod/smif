...
def results_no_results(empty_store):
    ...
    empty_store.write_dimension(
        {"name": "sample_dim", "elements": [{"name": "a"}, {"name": "b"}]}
    )
    empty_store.write_dimension(
        {
...
def results_with_results(results_no_results):
    ...
    results_no_results._store.write_results(
        sample_results, "model_run_1", "a_model", 2010, 0
    )
    results_no_results._store.write_results(
        sample_results, "model_run_1", "a_model", 2015, 0
    )
    results_no_results._store.write_results(
        sample_results, "model_run_1", "a_model", 2020, 0
    )
    results_no_results._store.write_results(
        sample_results, "model_run_1", "a_model", 2015, 1
    )
    results_no_results._store.write_results(
        sample_results, "model_run_1", "a_model", 2020, 1
    )
    results_no_results._store.write_results(
        sample_results, "model_run_1", "a_model", 2015, 2
    )
    results_no_results._store.write_results(
        sample_results, "model_run_1", "a_model", 2020, 2
    )

    results_no_results._store.write_results(
        sample_results, "model_run_1", "b_model", 2010, 0
    )
    results_no_results._store.write_results(
        sample_results, "model_run_1", "b_model", 2015, 0
    )
    results_no_results._store.write_results(
        sample_results, "model_run_1", "b_model", 2020, 0
    )
    results_no_results._store.write_results(
        sample_results, "model_run_1", "b_model", 2025, 0
    )
    results_no_results._store.write_results(
        sample_results, "model_run_1", "b_model", 2030, 0
    )

    results_no_results._store.write_results(
        sample_results, "model_run_2", "b_model", 2010, 0
    )
    results_no_results._store.write_results(
        sample_results, "model_run_2", "b_model", 2015, 0
    )
    results_no_results._store.write_results(
        sample_results, "model_run_2", "b_model", 2020, 0
    )
    results_no_results._store.write_results(
        sample_results, "model_run_2", "b_model", 2025, 0
    )
    results_no_results._store.write_results(
        sample_results, "model_run_2", "b_model", 2030, 0
    )
...
class TestNoResults:
    def test_exceptions(self, empty_store):
        ...
        with raises(ValueError) as ex:
            invalid_dir = os.path.join(
                os.path.dirname(__file__), "does", "not", "exist"
            )
    ...
    def test_list_sector_models(self, results_no_results):
        assert results_no_results.list_sector_models("model_run_1") == [
            "a_model",
            "b_model",
        ]
        assert results_no_results.list_sector_models("model_run_2") == [
            "a_model",
            "b_model",
        ]
    ...
    def test_list_scenario_outputs(self, results_no_results):
        ...
        assert results_no_results.list_scenario_outputs("a_scenario") == [
            "a_provides",
            "b_provides",
        ]
        ...
        expected = pd.DataFrame(
            OrderedDict(
                [
                    ...
                    (
                        "decision",
                        [0] * 12 + [1] * 6 + [2] * 6 + [0] * 6 + [1] * 6 + [2] * 6,
                    ),
                    ("sample_dim", ["a", "a", "a", "b", "b", "b"] * 7),
                    ("sample_dim_colour", ["red", "green", "blue"] * 14),
                    (
                        "sample_output",
                        np.asarray([1, 2, 3, 4, 5, 6] * 7, dtype=np.int32),
                    ),
                ]
            )
        )
        ...
        expected = pd.DataFrame(
            OrderedDict(
                [
                    ...
                    (
                        "sample_output",
                        np.asarray([1, 2, 3, 4, 5, 6] * 10, dtype=np.int32),
                    ),
                ]
            )
        )
...
class TestReadScenarios:
    def test_read_scenario_variant_data(
        self,
        results_no_results,
        model_run,
        sample_dimensions,
