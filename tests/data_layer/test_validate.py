...
class TestValidateSosModel:
    ...
    def test_sector_model_missing_reference(self, get_sos_model, sample_scenarios):
        ...
        """
        try:
            validate_sos_model_config(get_sos_model, [], sample_scenarios)
            assert False
        except SmifDataError as ex:
            assert ex.args[0]
            assert ex.args[0][0].component == "sector_models"
    ...
    def test_dependencies_circular(
        ...
        configuration
        """
        get_sos_model["sector_models"] = ["energy_demand"]
    ...
    def test_dependencies_source_or_sink_not_enabled(
        ...
        get_sos_model["sector_models"] = ["energy_supply"]
        get_sector_model["outputs"] = [
            {
                "name": "output_a",
                "dims": ["dim_a"],
    ...
    def test_dependencies_has_matching_specs(
        ...
        energy_supply_sector_model["inputs"] = [
            {"name": "input_a", "dims": ["dim_b"], "dtype": "dtype_a", "unit": "unit_a"}
        ]
        ...
        energy_supply_sector_model["inputs"] = [
            {"name": "input_a", "dims": ["dim_a"], "dtype": "dtype_b", "unit": "unit_a"}
        ]
    ...
    def test_dependencies_with_no_dims_spec(
        ...
        energy_supply_sector_model["inputs"] = [
            {"name": "input_a", "dtype": "dtype_a", "unit": "unit_a"}
        ]
        ...
        try:
            ...
            assert ex.args[0][0].component == "model_dependencies"
            assert (
                "Source `output_a` has different dimensions than sink `input_a`"
                in ex.args[0][0].error
            )
