...
@fixture(scope='function')
def get_narratives_config():
    """Return minimum narratives config
    """
    return [
    {
        "name": "narrative_1",
...
class TestValidateSosModel:
    ...
    def test_correct(
        ...
        get_scenarios_config, get_narratives_config):
    ...
    def test_narrative_missing_reference(
        self, get_sos_models_config, get_sector_models_config, 
        get_scenarios_config, get_narratives_config):
        """Expect error when references narrative 
        configuration does not exist
        """
        try:
            get_sos_models_config[0]['narratives'] = ['narrative_a']
            get_narratives_config = []
            validate_sos_model_config(
                get_sos_models_config[0], 
                get_sector_models_config,
            ...
            assert len(ex.args[0]) == 1
            assert ex.args[0][0].component == 'narratives'
            assert 'valid narrative configuration' in ex.args[0][0].error
