...
class TestDataArray:
    ...
    def test_read_zero_d_from_timeseries(self, handler):
        ...
        read_spec = Spec(name="param", dims=[], coords={}, dtype="float")
...
class TestResults:
    ...
    def test_read_write_results(self, handler, sample_results):
        ...
        results_out = handler.read_results(
            modelrun_name, model_name, output_spec, timestep
        )
    ...
    def test_available_results(self, handler, sample_results):
        ...
        handler.delete_results(
            "test_modelrun", "energy", sample_results.spec.name, 2010, 0
        )
        assert sorted(handler.available_results("test_modelrun")) == [
            (2015, 0, "energy", sample_results.spec.name),
            (2015, 1, "energy", sample_results.spec.name),
        ...
        handler.delete_results(
            "test_modelrun", "energy", sample_results.spec.name, 2015, 0
        )
        handler.delete_results(
            "test_modelrun", "energy", sample_results.spec.name, 2015, 1
        )
    ...
    def test_read_results_raises(self, handler, sample_results):
        ...
        handler.write_results(
            sample_results, modelrun_name, model_name, timestep=timestep
        )
...
class TestDataExists:
    ...
    def test_strategy_data_exists(self, handler, strategies):
        for strategy in strategies:
            if strategy["type"] == "pre-specified-planning":
                ...
                handler.write_strategy_interventions(
                    new_strategy, strategy["interventions"]
                )
    ...
    def test_scenario_variant_data_exists(self, handler, sample_scenario_data):
        ...
        handler.write_scenario_variant_data(
            "scenario_variant_data", scenario_variant_data
        )
    ...
    def test_narrative_variant_data_exists(self, handler, sample_narrative_data):
        ...
        handler.write_narrative_variant_data(
            "narrative_variant_data", narrative_variant_data
        )

        assert handler.narrative_variant_data_exists("narrative_variant_data")

    def test_model_parameter_default_data_exists(
        self, handler, get_multidimensional_param
    ):
