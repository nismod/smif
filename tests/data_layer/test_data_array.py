...
class TestDataFrameInterop:
    def test_to_from_df(self):
        df = pd.DataFrame([{"test": 3, "region": "oxford", "interval": 1}]).set_index(
            ["region", "interval"]
        )
        ...
        da = DataArray(spec, numpy.array([[3.0]], dtype="int64"))
    ...
    def test_single_dim_order(self):
        spec = Spec(
            ...
            coords={
                "technology_type": ["water_meter", "electricity_meter", "other", "aaa"]
            },
            dtype="float",
        )
        df = pd.DataFrame(
        ...
        da = DataArray(spec, numpy.array([5.0, 6.0, 7.0, 8.0]))
        da_from_df = DataArray.from_df(spec, df)
    ...
    def test_df_round_trip(self):
        ...
        df = pd.DataFrame(
            [
                {"technology_type": "water_meter", "multi_savings": 5.0},
                {"technology_type": "electricity_meter", "multi_savings": 6.0},
                {"technology_type": "other", "multi_savings": 7.0},
                {"technology_type": "aaa", "multi_savings": 8.0},
            ]
        )
    ...
    def test_match_metadata(self):
        spec = Spec(
            name="test", dims=["region"], coords={"region": ["oxford"]}, dtype="int64"
        )

        # must have a column named the same as the spec.name
        df = pd.DataFrame([{"region": "oxford", "other": "else"}]).set_index(["region"])
        msg = (
        ...
        df = pd.DataFrame(
            [{"test": 3.14, "region": "oxford"}, {"test": 3.14, "region": "extra"}]
        ).set_index(["region"])
        msg = (
            "Data for 'test' contained unexpected values in the set of coordinates for "
...
class TestMissingData:
    def test_missing_data_raises(self, small_da):
        ...
        msg = (
            "Data for 'test_data' had missing values - read 20 but expected 24 in "
    ...
    def test_missing_data_message(self, small_da):
        ...
        expected = (
            "Data for 'test_data' had missing values - read 22 but expected 24 in "
            + "total, from dims of length {a: 2, b: 3, c: 4}"
        )
    ...
    def test_missing_data_non_numeric(self, small_da_non_numeric):
        ...
        df = small_da_non_numeric.as_df()
        actual = show_null(df)
        actual.test_data = actual.test_data.astype(
            float
        )  # coerce to float to handle NaN consistently as StringDtype is experimental
        levels = [["a1", "a2"], ["b1", "b2", "b3"], ["c1", "c2", "c3", "c4"]]
        codes = [[1], [1], [1]]
        names = ["a", "b", "c"]
        try:
        ...
        expected = pd.DataFrame(
            data=numpy.array([[None]], dtype=float),
            index=index,
            columns=["test_data"],
        )
        pd.testing.assert_frame_equal(actual, expected)
