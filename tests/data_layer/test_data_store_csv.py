...
class TestScenarios:
    ...
    def test_missing_timestep(
        ...
        msg = (
            "Data for 'population_count' expected a column called 'timestep', instead "
            + "got data columns ['population_count', 'county', 'season', 'year'] and "
            + "index names [None]"
        )
        assert msg in str(ex.value)

    def test_data_column_error(
        self, setup_folder_structure, config_handler, scenario_spec, get_scenario_data
    ):
        data = []
        ...
        key = _write_scenario_csv(
            setup_folder_structure, data, ("population_count", "region", "season")
        )

        with raises(SmifDataMismatchError) as ex:
            config_handler.read_scenario_variant_data(key, scenario_spec)
        msg = (
            "Data for 'population_count' expected a data column called "
    ...
    def test_scenario_data_validates(
        self,
        setup_folder_structure,
        config_handler,
    ...
    def test_missing_data_gives_nan(
        self,
        setup_folder_structure,
        config_handler,
        scenario_spec,
        get_missing_scenario_data,
        caplog,
    ):
        key = _write_scenario_csv(
            setup_folder_structure,
...
class TestNarrativeVariantData:
    ...
    def test_default_data_mismatch(
        self, config_handler, get_sector_model_parameter_defaults
    ):
        ...
        msg = (
            "Data for 'smart_meter_savings' should contain a single value, instead got "
    ...
    def test_error_duplicate_rows_multi_index(self, config_handler):
        spec = Spec(
            name="test", dims=["a", "b"], coords={"a": [1, 2], "b": [3, 4]}, dtype="int"
        )
        path = os.path.join(config_handler.data_folders["parameters"], "default.csv")
        with open(path, "w") as csvfile:
    ...
    def test_error_wrong_name(self, config_handler):
        ...
        with open(path, "w") as csvfile:
            ...
            writer.writeheader()
            writer.writerow({"a": 1, "b": 2, "wrong_name": 0})

        with raises(SmifDataMismatchError) as ex:
            config_handler.read_model_parameter_default("default", spec)

        msg = (
            "Data for 'test' expected a data column called 'test' and index names "
    ...
    def test_error_not_full(self, config_handler):
        ...
        with open(path, "w") as csvfile:
            writer = csv.DictWriter(csvfile, fieldnames=["test", "a", "b"])
            writer.writeheader()
            writer.writerow({"a": 1, "b": 3, "test": 0})

        with raises(SmifDataMismatchError) as ex:
            config_handler.read_model_parameter_default("default", spec)

        msg = (
            "Data for 'test' had missing values - read 1 but expected 4 in total, from "
...
class TestWarmStart:
    ...
    def test_prepare_warm_start(self, setup_folder_structure):
        ...
        previous_results_path = os.path.join(
            str(setup_folder_structure), "results", modelrun, model, "decision_none"
        )
        os.makedirs(previous_results_path, exist_ok=True)

        path = os.path.join(
            previous_results_path, "output_electricity_demand_timestep_2020.csv"
        )
        with open(path, "w") as fh:
            fh.write("region,interval,value\noxford,1,4.0\n")

        path = os.path.join(
            previous_results_path, "output_electricity_demand_timestep_2025.csv"
        )
        with open(path, "w") as fh:
            fh.write("region,interval,value\noxford,1,6.0\n")

        path = os.path.join(
            previous_results_path, "output_electricity_demand_timestep_2030.csv"
        )
        with open(path, "w") as fh:
            fh.write("region,interval,value\noxford,1,8.0\n")
    ...
    def test_prepare_warm_start_other_local_storage(self, setup_folder_structure):
        ...
        # Create results for a 'previous' modelrun
        previous_results_path = os.path.join(
            str(setup_folder_structure), "results", modelrun, model
        )
        os.makedirs(previous_results_path, exist_ok=True)

        path = os.path.join(
            previous_results_path, "output_electricity_demand_timestep_2020.parquet"
        )
        with open(path, "w") as fh:
            fh.write("region,interval,value\noxford,1,4.0\n")

        path = os.path.join(
            previous_results_path, "output_electricity_demand_timestep_2025.parquet"
        )
        with open(path, "w") as fh:
            fh.write("region,interval,value\noxford,1,6.0\n")

        path = os.path.join(
            previous_results_path, "output_electricity_demand_timestep_2030.parquet"
        )
        with open(path, "w") as fh:
            fh.write("region,interval,value\noxford,1,8.0\n")
