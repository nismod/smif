...
def register(
    regions_half_triangles,
    regions_half_squares,
    regions_single_half_square,
    regions_rect,
):
...
class TestRegionAdaptor:
    ...
    def test_aggregate_region(self, regions_rect, regions_half_squares):
        ...
        to_spec = Spec(
            name="test-var", dtype="float", dims=["rect"], coords={"rect": regions_rect}
        )
    ...
    def test_half_to_one_region_pass_through_time(
        self, regions_rect, regions_half_squares
    ):
        ...
        from_spec = Spec(
            ...
            dtype="float",
            dims=["rect", "months"],
            coords={"rect": regions_rect, "months": list(range(12))},
        )
        adaptor.add_input(from_spec)
        to_spec = Spec(
            name="test-var",
            ...
            coords={"half_squares": regions_half_squares, "months": list(range(12))},
        )
        adaptor.add_output(to_spec)
...
class TestRegionRegister:
    ...
    def test_convert_to_half_not_covered(self, register, caplog):
        ...
        expected = (
            "Coverage for 'rect' is 2 and does not match "
    ...
    def test_convert_from_half_not_covered(self, register, caplog):
        ...
        expected = (
            "Coverage for 'single_half_square' is 1 and does not "
