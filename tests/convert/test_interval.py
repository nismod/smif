...
class TestTimeRegisterConversion:
    ...
    def test_remap_months_to_mapped_months(
        self, months, monthly_data, remap_months, remap_month_data
    ):
        register = NDimensionalRegister()
        register.register(IntervalSet("months", months))
        register.register(IntervalSet("remap_months", remap_months))
...
class TestConvertor:
    ...
    def test_one_region_time_aggregation(self, months, seasons, regions_rect):
        ...
        expected = np.array(
            [[31 + 31 + 28, 31 + 30 + 31, 30 + 31 + 31, 30 + 31 + 30]], dtype=float
        )  # area a, seasons 1-4
        ...
        from_spec = Spec(
            ...
            dtype="float",
            dims=["rect", "months"],
            coords={"months": months, "rect": regions_rect},
        )
        adaptor.add_input(from_spec)
        to_spec = Spec(
            name="test-var",
            dtype="float",
            dims=["rect", "seasons"],
            coords={"seasons": seasons, "rect": regions_rect},
        )
        adaptor.add_output(to_spec)
    ...
    def test_one_region_convert_from_hour_to_day(
        self, regions_rect, twenty_four_hours, one_day
    ):
        ...
        from_spec = Spec(
            ...
            coords={"twenty_four_hours": twenty_four_hours, "rect": regions_rect},
        )
        adaptor.add_input(from_spec)
        to_spec = Spec(
            name="test-var",
            dtype="float",
            dims=["rect", "one_day"],
            coords={"one_day": one_day, "rect": regions_rect},
        )
        adaptor.add_output(to_spec)
...
class TestInterval:
    ...
    def test_to_hours_list(self):
        interval = Interval(
            "test", [("PT0H", "PT1H"), ("PT2H", "PT3H"), ("PT5H", "PT7H")]
        )
        actual = interval.bounds
    ...
    def test_str_one_interval(self):
        interval = Interval("test", ("P2M", "P3M"))
        actual = str(interval)
        assert actual == "Interval 'test' maps to:\n  hour 1416 to hour 2160\n"
    ...
    def test_load_remap_timeslices(self):
        interval = Interval("1", [("P2M", "P3M"), ("P3M", "P4M"), ("P4M", "P5M")])
        actual = interval.bounds
...
class TestIntervalRegister:
    ...
    def test_conversion_with_different_coverage_fails(self, one_year, one_day, caplog):
        ...
        expected = (
            "Coverage for 'one_year' is 8760 and does not match "
...
class TestValidation:
    def test_validate_get_hourly_array(self, remap_months):
        ...
        expected = np.ones(8760, dtype=np.int32)
